\makeatletter\let\latex@xfloat\@xfloat\makeatother

\documentclass{ruthesis}
\special{papersize=8.5in,11in} % for A4-default configurations on servers

\usepackage{etoolbox}
\makeatletter
\let\@xfloat\latex@xfloat
\apptocmd{\@xfloat}{\linespread{1}\normalsize}{}{}
\makeatother

\input{packages}
\input{defines}

\begin{document}
\phd

\title{Understanding Dictionaries at the Intersection of Theory and Practice}
\author{Alexander Conway}
\program{Computer Science}
\director{Mart\'in Farach-Colton}
\approvals{4}
\submissionyear{2020}
\submissionmonth{May}

\input{abstract}

\beforepreface
\input{acknowledgements}
%\dedication{The body of the dedication}
\afterpreface

\chapter{Introduction}
\input{intro}

\chapter{Optimal Hashing in External Memory}
\input{boa-intro}
\input{boa-prelim}
\input{boa-boa}
\input{boa-refined}
\input{boa-bot}
\input{boa-cacheoblivious}
\input{boa-asymmetric}

\chapter{SplinterDB}
\input{splinter-intro}
\input{splinter-background}
\input{splinter-design}
\input{splinter-flushcompact}
\input{splinter-splitting}
\input{splinter-details}
\input{splinter-eval}
\input{splinter-related}

\chapter{File System Aging}
\input{fsa-intro}
\input{fsa-related}
\input{fsa-mitigation}
\input{fsa-model}
\input{fsa-plots}
\input{fsa-metrics}
\input{fsa-setup}
\input{fsa-microbenchmarks}
\input{fsa-git}
\input{fsa-mailserver}
\input{fsa-conclusion}

\chapter{Optimal Ball Recycling}
\input{br-intro}
\input{br-motivation}
\input{br-prelim}
\input{br-nonuniform}
\input{br-uniform}
\input{br-experiments}

\bibliographystyle{plainurl}
\bibliography{./bibliography}
\end{document}
