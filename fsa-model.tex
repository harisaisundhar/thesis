...
In~\secref{git}, we show that the aging of \btrfs is inversely
...
%is that generally such
%blocks will be quite large---4\mb is reasonable for hard drives for example---so
%that when making small writes and updates, a large block must be
%re-sequentialized and written. Thus a small logical write ends up generating a
%large write to disk, a process known as ``write amplification.''
...
%We
%experimentally demonstrate the tradeoff of block size on both read performance
%and write amplification when using \btrfs in 
