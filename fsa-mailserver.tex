\section{Application Level Aging: Mail Server}\label{sec:fsa-mailserver}
...
%Dovecot is
%an open source IMAP and POP3 email server for LINUX/UNIX, and Maildir
%stores each mail as a file.
We simulate 2 users, each having 80 mailboxes
receiving new email, deleting old emails, and searching through
their mailboxes. 
%We simulate this behavior for 2 users, each having 80
%mailboxes.
...
deletes are balanced, mailbox size tends to stay around 1,000.  \fixmedp{Is
this right?} We simulate the mailserver for 100 cycles and after each cycle we
...
\figref{mailserver} shows the read costs in seconds per GiB of the grep test on
...
%and by a factor of 15 compared to the fastest file systems \fixmeac{ get real
%factor here },
...
%and after running the test for 3 weeks, we were only able to get as far as
%4,300 iterations.
...
%
% space constraints we have not included a figure with the dynamic layout
%scores, however, these are similarly correlated with the read performance
%shown in \figref{mailserver}.

%At the time of submission, we were not able to complete the \zfs clean
%test, as it regularly crashed.  The \betrfs{} clean results were also not complete
%at the time of submission, but were consistent with the aged version.
%This experiment was also conducted on a slightly different machine setup, using the first
%partition of the hard drive to store the root file system image, and 
%We spot checked these results against the default setup (booting from a USB device)
%but were not able to get a complete dataset at the time of submission.

%Due to unforeseen circumstances, we do not currently show the \betrfs
%clean version, although preliminary testing showed it to be almost identical
%to the aged one.
...
%
\newcommand{\mailserverDataDir}{../data/mailserver_aging}
